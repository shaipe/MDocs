...
import { isString, appendUrlParams, deepMerge, isFunction, isObject } from '@htui/utils';
// import { useI18n } from '@htui/locale';
import { context } from '../bridge';
import { RequestOptions, RequestResult } from '@htui/types';
...
const transform: AxiosTransform = {
    ...
    transformRequestHook: (res: AxiosResponse<RequestResult>, options: RequestOptions) => {
        ...
        if (!data) {
            ...
            throw new Error('请求失败'); // t('sys.api.apiRequestFailed'));
        }
        // 后台直接返回的字符串时不作任何处理
        ...
        // 	context.modalFunction({
        // 		title: t('sys.api.errorTip'),
        // 		content: timeoutMsg,
        // 	});
        // } else if (options.errorMessageMode === 'message') {
        // 	context.msgFunction(timeoutMsg);
        // }
    ...
    requestInterceptors: (config, options) => {
        ...
        // 请求头处理
        if (isFunction(context.headerFunction)) {
            const header = context.headerFunction();
            if (header) {
    ...
    responseInterceptorsCatch: (error: any) => {
        ...
        try {
            if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
                errMessage = '请求超时'; // t('sys.api.apiTimeoutMessage');
            }
            if (err?.includes('Network Error')) {
                errMessage = '网络异常'; // t('sys.api.networkExceptionMsg');
            }

            if (errMessage) {
                if (errorMessageMode === 'modal') {
                    context.modalFunction({
                        title: '错误', // t('sys.api.errorTip'),
                        content: errMessage,
                    });
                } else if (errorMessageMode === 'message') {
                    context.msgFunction(errMessage);
                }
